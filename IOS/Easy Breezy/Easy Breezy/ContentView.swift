    ...
    class BluetoothManager: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
        ...
        func sendPacket(type: UInt32, value: Float) {
            guard let peripheral = selectedDevice,
                  let characteristic = characteristic else {
                addDebugLog("❌ Not ready to send packets")
                return
            }
            ...
            var packetData = Data()
            packetData.append(Data(bytes: &typeLE, count: MemoryLayout<UInt32>.size))
            packetData.append(Data(bytes: &valueLE, count: MemoryLayout<UInt32>.size))
        ...
        func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
            ...
            // Just use the first characteristic we find
            if let firstCharacteristic = characteristics.first {
                self.characteristic = firstCharacteristic
                addDebugLog("✅ Ready to send packets")
                
                // Send test packet immediately after finding characteristic
                sendPacket(type: 1, value: 22.5)
            }
        }
    }
